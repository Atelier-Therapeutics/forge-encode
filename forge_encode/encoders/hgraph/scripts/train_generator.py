...
for epoch in range(args.epoch):
    ...
    for batch in tqdm(dataset):
        ...
        meters = meters + np.array([
            kl_div.detach().cpu().item() if torch.is_tensor(kl_div) else kl_div,
            loss.item() if torch.is_tensor(loss) else loss,
            wacc.detach().cpu().item() if torch.is_tensor(wacc) else wacc,
            iacc.detach().cpu().item() if torch.is_tensor(iacc) else iacc,
            tacc.detach().cpu().item() if torch.is_tensor(tacc) else tacc,
            sacc.detach().cpu().item() if torch.is_tensor(sacc) else sacc
        ])
