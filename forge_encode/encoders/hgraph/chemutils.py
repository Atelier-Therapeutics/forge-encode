...
def expand_to_complete_aromatic_systems(mol, atoms):
    """
    Expand a set of atoms to include complete aromatic systems.
    This ensures that if any atom in the fragment is aromatic, 
    the entire aromatic system is included.
    """
    atoms = set(atoms)
    expanded = set(atoms)
    ...
    # If any of our atoms are aromatic, we need to include the complete aromatic system
    if atoms & aromatic_atoms:
        # Find all aromatic rings
        aromatic_rings = []
        for ring in Chem.GetSymmSSSR(mol):
            ring_atoms = set(ring)
            # Check if this ring contains any of our atoms
            if ring_atoms & atoms:
                aromatic_rings.append(ring_atoms)
        
        # Expand to include all atoms in aromatic rings that contain our atoms
        for ring_atoms in aromatic_rings:
            expanded.update(ring_atoms)
    
    return list(expanded)

def get_clique_mol(mol, atoms):
    """
    Get a molecular fragment, expanding to include complete aromatic systems
    to avoid kekulization errors. If the fragment cannot be represented as a valid molecule,
    ...
    try:
        ...
        if new_mol is None:
            print(f"[WARN] Fragment with atoms {atoms} could not be sanitized after kekulization.")
        return new_mol
    except Chem.rdchem.KekulizeException:
        expanded_atoms = expand_to_complete_aromatic_systems(mol, atoms)
        if expanded_atoms != atoms:
            try:
                smiles = Chem.MolFragmentToSmiles(mol, expanded_atoms, kekuleSmiles=True)
                new_mol = Chem.MolFromSmiles(smiles, sanitize=False)
                new_mol = copy_edit_mol(new_mol).GetMol()
                new_mol = sanitize(new_mol)
                if new_mol is not None:
                    ...
                    print(f"[WARN] Expanded fragment with atoms {expanded_atoms} could not be sanitized after kekulization.")
            except Chem.rdchem.KekulizeException:
                pass
        # Fallback: try non-kekulized
        smiles = Chem.MolFragmentToSmiles(mol, expanded_atoms, kekuleSmiles=False)
        new_mol = Chem.MolFromSmiles(smiles, sanitize=False)
        if new_mol is None:
            print(f"[WARN] Fragment with atoms {expanded_atoms} could not be represented as a molecule (even without kekulization). Skipping.")
            return None
        new_mol = copy_edit_mol(new_mol).GetMol()
        new_mol = sanitize(new_mol)
        if new_mol is None:
            print(f"[WARN] Fragment with atoms {expanded_atoms} could not be sanitized after fallback. Skipping.")
            return None
        return new_mol
