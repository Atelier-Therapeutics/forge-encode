...
from forge_encode.encoders.hgraph.rnn import GRU, LSTM
...
class HierMPNEncoder(nn.Module):
    ...
    def __init__(self, vocab, avocab, rnn_type, embed_size, hidden_size, depthT, depthG, dropout):
        ...
        # Create embedding matrices but don't move to device yet - will be done in forward pass
        self.register_buffer('E_a', torch.eye(atom_size))
        self.register_buffer('E_b', torch.eye(len(MolGraph.BOND_LIST)))
        self.register_buffer('E_apos', torch.eye(MolGraph.MAX_POS))
        self.register_buffer('E_pos', torch.eye(MolGraph.MAX_POS))
...
class IncHierMPNEncoder(HierMPNEncoder):
    ...
    def get_sub_tensor(self, tensors, subset):
        ...
        subnode = subnode.to(device)
        submess = submess.to(device)
        ...
        if len(tensors) == 6:
            cgraph = tensors[4].to(device).index_select(0, subnode)
