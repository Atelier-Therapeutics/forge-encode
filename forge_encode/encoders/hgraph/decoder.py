...
from forge_encode.encoders.hgraph.nnutils import *
from forge_encode.encoders.hgraph.encoder import IncHierMPNEncoder
from forge_encode.encoders.hgraph.mol_graph import MolGraph
from forge_encode.encoders.hgraph.inc_graph import IncTree, IncGraph
...
class HierMPNDecoder(nn.Module):
    ...
    def apply_tree_mask(self, tensors, cur, prev):
        ...
        device = cur.emask.device
        agraph = agraph.to(device) * index_select_ND(cur.emask, 0, agraph.to(device))
        bgraph = bgraph.to(device) * index_select_ND(cur.emask, 0, bgraph.to(device))
        cgraph = cgraph.to(device) * index_select_ND(prev.vmask.to(device), 0, cgraph.to(device))
    ...
    def apply_graph_mask(self, tensors, hgraph):
        ...
        # Ensure all tensors are on the same device
        device = hgraph.emask.device
        agraph = agraph.to(device) * index_select_ND(hgraph.emask, 0, agraph.to(device))
        bgraph = bgraph.to(device) * index_select_ND(hgraph.emask, 0, bgraph.to(device))
    ...
    def get_cls_score(self, src_tree_vecs, batch_idx, cls_vecs, cls_labs):
        ...
        if cls_labs is None: #inference mode
            ...
            icls_scores = self.iclsNN(cls_vecs) + vocab_masks.to(cls_vecs.device) #apply mask by log(x + mask): mask=0 or -INF
    ...
    def get_assm_score(self, src_graph_vecs, batch_idx, assm_vecs):
        if self.use_attention:
            ...
            assm_cxt = index_select_ND(src_graph_vecs, 0, batch_idx.to(src_graph_vecs.device))
    ...
    def forward(self, src_mol_vecs, graphs, tensors, orders):
        ...
        for t in range(maxt):
            ...
            # Ensure tensors are created on the correct device for each emask
            device_htree = htree.emask.device
            device_hinter = hinter.emask.device
            subtree0 = torch.tensor(subtree[0], device=device_htree)
            subtree1_htree = torch.tensor(subtree[1], device=device_htree)
            subtree1_hinter = torch.tensor(subtree[1], device=device_hinter)
            htree.emask.scatter_(0, subtree1_htree, 1)
            hinter.emask.scatter_(0, subtree1_hinter, 1)
            subtree = (subtree0, subtree1_htree)
        ...
        if len(all_assm_preds) > 0:
            ...
            assm_loss = torch.tensor(0.0, device=cls_acc.device)
            assm_acc = torch.tensor(1.0, device=cls_acc.device)
    ...
    def enum_attach(self, hgraph, cands, icls, nth_child):
        ...
        # Move icls_vecs to the same device as E_assm embedding weights
        icls_vecs = icls_vecs.to(self.E_assm[0].weight.device)
        icls_vecs = self.E_assm(icls_vecs)

        nth_child = self.itensor.new_tensor([nth_child] * len(cands.view(-1)))
        order_vecs = self.E_order.index_select(0, nth_child.to(self.E_order.device))

        cand_vecs = hgraph.node.index_select(0, cands.view(-1).to(hgraph.node.device))
        device = cand_vecs.device
        cand_vecs = cand_vecs.to(device)
        icls_vecs = icls_vecs.to(device)
        order_vecs = order_vecs.to(device)
